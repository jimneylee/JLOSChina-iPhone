...
- (void)savePhoto
{
	if (self.isOriginPhotoLoaded && self.imageView.image) {
        ...
        [OSCGlobalConfig HUDShowMessage:@"图片未下载完成，无法保存" addedToView:self];
    }
}
...
{
    if (error) {
        [OSCGlobalConfig HUDShowMessage:@"保存失败" addedToView:self];
    }
    else {
...
{
    [OSCGlobalConfig HUDShowMessage:@"原始大图加载失败！" addedToView:self];
}
